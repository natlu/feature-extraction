...
setMethod(f="getImageFeatures",
          ...
          definition=function(feature_obj, parallel = TRUE, cores = 3)
          {
            ...
            imageFeatures <- feature_obj@dat[, .(idx, pdfFiles, directory, name)]
            imageFeatures[, imgRegex  := paste0(name, "(-[0-9]+|).png")]
            ...
            foofunc <- function(x) {
              ...
              for (i in 1:nrow(x)) {
                output[[i]] <- getImgFeatures(x[i, pngFiles])
              }
              output <- rbindlist(output)
              return(cbind(x[, .(idx)], output))
            }
            ...
            if (parallel) {
              ...
              pngSplit <- split(imageFeatures, ceiling(seq_along(imageFeatures$pngFiles)/10))
...
################################################################################
# getting the means for each group/pdf
# foo[, lapply(.SD, mean), by = idx, .SDcols = c("coldistr0", "coldistr1")]
################################################################################
...
# 1402/7 = 200, 993/5 = 199
  ...
  # Get the page number of the png file
  page <- str_extract(pngFile, "(?<=-)([0-9]+)(?=.png)")
  page <- ifelse(is.na(page), 1, as.numeric(page) + 1)

  output <- data.table(page = page, t(c(imgDistrCols, imgDistrRows, imgDistrColsA, subMatDensity)))
